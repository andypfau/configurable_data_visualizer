...
for i_file in range(3):
    ...
    freq_hz, p_target_dbm, predistortion, vdd = (mesh.ravel() for mesh in np.meshgrid(
        np.round(np.arange(1e9, 10e9+ε, 100e6), 3), # frequency
        np.round(np.arange(-30, +10+ε, 10), 3), # power
        np.round(np.arange(-0.01, +0.01+ε, 0.002), 5), # comp
        np.round(np.arange(6, 8+ε, 1), 3), # supply
    ))
    ...
    def calc_fundamental_and_intermod(level_dbm, sat_level_dbm, predistortion):
        ...
        fundamental_level_mw = compress(level_mw)
        intermod_level_mw = compress(3/4*c3*level_mw**3) + predistortion*level_mw**3
    ...
    p_sat = 7 + vdd/2
    p_fundamental_dbm, p_intermod_dbm = calc_fundamental_and_intermod(p_input_dbm, p_sat, predistortion)
